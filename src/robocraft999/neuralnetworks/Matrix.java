...
public class Matrix{
	  ...
	  public Matrix(int rows_, int cols_){
	    this.rows = rows_;
	    this.cols = cols_;
	    if(rows_ == 0 || cols_ == 0)System.out.println("WARNING: rows and cols should not be zero");
	...
	Option 2:
	Func test = (x) -> {return x * 3;}; //easier but not availible in this java version
	*/
	interface Func{
	  float f(float x);
	}
